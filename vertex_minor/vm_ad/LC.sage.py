...
# This file was *autogenerated* from the file LC.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_4 = Integer(4); _sage_const_3 = Integer(3); _sage_const_0 = Integer(0)
...
def E(i,n):
    ...
    tmp=zero_matrix(GF(_sage_const_2 ),n)
    tmp[i,i]=_sage_const_1 
    return tmp
...
def constr_mat(A1,A2):
    ...
    mat1=matrix(GF(_sage_const_2 ),map(lambda i:M2vec(M_tim_E(A1,i)),range(n)))
    mat2=matrix(GF(_sage_const_2 ),map(lambda i:M2vec(M_tim_E(A1,i)*A2),range(n)))
    mat3=matrix(GF(_sage_const_2 ),map(lambda i:M2vec(E(i,n)),range(n)))
    mat4=matrix(GF(_sage_const_2 ),map(lambda i:M2vec(E_tim_M(A2,i)),range(n)))
    return block_matrix(GF(_sage_const_2 ),[[mat1],[mat2],[mat3],[mat4]],subdivide=False).transpose()
...
def cond(vec):
    ...
    n=vec.length()/_sage_const_4 
    # return all([not matrix(GF(2),[[vec[a],vec[n+a]],[vec[2*n+a],vec[3*n+a]]]).is_singular() for a in range(n)])
    return all(map(lambda a:vec[a]*vec[_sage_const_3 *n+a]+vec[n+a]*vec[_sage_const_2 *n+a]!=_sage_const_0 ,range(n)))
