...
# This file was *autogenerated* from the file GraphClasses.sage
...
class SimpleGraph(Graph):
    def __init__(self,*args,**kwargs):
        ...
        try:
            ...
            try:
                self._order=list(args[_sage_const_0 ].get_order())
            except:
                self._order=self.vertices()
        except StandardError as e:
            try:
                data=list(args[_sage_const_0 ])
                n=len(data)
                if kwargs.has_key('format'):
                    if kwargs['format']=='empty':
                        A=zero_matrix(GF(_sage_const_2 ),n)
                    else:
                        A=ones_matrix(GF(_sage_const_2 ),n)+identity_matrix(GF(_sage_const_2 ),n)
                else:
                    A=ones_matrix(GF(_sage_const_2 ),n)+identity_matrix(GF(_sage_const_2 ),n)
    ...
    def __add__(self,other):
        ...
        G=SimpleGraph(matrix(GF(_sage_const_2 ),Gself.adjacency_matrix(vertices=vert_order))+matrix(GF(_sage_const_2 ),Gother.adjacency_matrix(vertices=vert_order)))
    ...
    def meas_X(self,a,b=None,inplace=False, return_special=False):
        ...
        if inplace:
            ...
            G=SimpleGraph(self)
        if len(G.neighbors(a))==_sage_const_0 :
            G.delete_vertex(a)
            bp = None
        else:
            if b==None:
                bp=G.neighbors(a)[_sage_const_0 ]
            else:
                bp=b
    ...
    def meas_seq(self,inds,bases,ret_meas=False,inplace=False):
        ...
        for (i,b) in zip(inds,bases):
            if b=="Y":
                ...
                if ret_meas:
                    if len(G.neighbors(i))==_sage_const_0 :
                        meas+="x"
                    else:
                        meas+=str(G.neighbors(i)[_sage_const_0 ])
    ...
    def has_VM(self,other,method='brute',check_DH=False):
        ...
        if method=='brute':
            ...
            m=[]
            for base in bases:
                if base[_sage_const_1 ]=='Z':
                    pass
                elif base[_sage_const_1 ]=='Y':
                    m.append(base[_sage_const_0 ])
                else:
                    m.append(base[_sage_const_0 ])
                    m.append(int(base[_sage_const_1 ]))
                    m.append(base[_sage_const_0 ])
            return m
        elif method=='DH':
            if check_DH:
            ...
            n=len(other)
            if not degrees==Set([_sage_const_1 ,n-_sage_const_1 ]):
    ...
    def can_meas(self,other,inds=None,start=True):
        ...
        try:
            other=graphs.StarGraph(other-_sage_const_1 )
        except:
            pass
        ...
        if (not self_connected) and other_connected:
            ...
            try:
                ...
                return (False,None)
        if len(inds)==_sage_const_0 :
            if self.is_LC_eq(other):
                return (True,[])
            return (False,None)
        noneq=[]
        for bas in ["Z","Y","X"]:
            (post,meas)=self.meas_seq(inds[:_sage_const_1 ],[bas],ret_meas=True)
            if post.is_connected() or (not other_connected):
                if not any(post.is_LC_eq(x) for x in noneq):
                    (ans,bases)=post.can_meas(other,inds=inds[_sage_const_1 :],start=False)
                    if ans:
                        bases=[[inds[_sage_const_0 ],meas]]+bases
                        return (True,bases)
                    noneq+=[post]
        return (False,None)
    def can_meas_all(self,other,inds=None):
        ...
        for bases in Tuples("ZYX",len(inds)):
            (post,meas)=self.meas_seq(inds,bases[::-_sage_const_1 ],ret_meas=True)
    ...
    def connect_leaves(self,V):
        ...
        try: #if V is just the number of vert, makes V=[0,1,2,,,,]
            ...
            V=list(V)
        if len(V)<=_sage_const_1 :
            return ([],'star')
        G=SimpleGraph(self)
        t_seq=G.can_meas_DH(V[:-_sage_const_1 ],check_rwd=False)
        G=(G*t_seq)
        Gsub=G.subgraph(V[:-_sage_const_1 ])
        for c in Gsub:
            if Gsub.degree(c)==(len(V)-_sage_const_2 ):
                break
        P=G.shortest_path(V[-_sage_const_1 ],c)
        if not P:
            raise RuntimeError("Target vertices not connected in graph")
        for v in P[_sage_const_1 :-_sage_const_1 ]:
            leaf_ns=[l for l in V[:-_sage_const_1 ] if (l!=c and G.has_edge(V[-_sage_const_1 ],l))]
            if leaf_ns:
                l=leaf_ns.pop()
                G.tau(l,inplace=True)
                t_seq.append(l)
            else:
                G.tau_seq([V[-_sage_const_1 ],v,V[-_sage_const_1 ]],inplace=True)
                t_seq+=[V[-_sage_const_1 ],v,V[-_sage_const_1 ]]
        dgr_set=Set(G.subgraph(V).degree())
        if dgr_set==Set([_sage_const_1 ,len(V)-_sage_const_1 ]):
            conf='star'
        elif dgr_set==Set([len(V)-_sage_const_1 ]):
            t_seq.append(c)
            conf='star'
        else:
            conf='notstar'
    ...
    def is_good(self,c,V,com):
        ...
        CC=[v for v in Gp if Gp.degree(v)>_sage_const_0 ]
        L=[v for v in Gp if Gp.degree(v)==_sage_const_0 ]
    ...
    def can_meas_DH(self,V,check_rwd=False):
        ...
        G=SimpleGraph(self)
        if check_rwd:
            if G.rank_decomposition()[_sage_const_0 ]!=_sage_const_1 :
        ...
        if conf=='star':
            ...
            Gsub=G.subgraph(V)
            for c in Gsub:
                if Gsub.degree(c)==k-_sage_const_1 :
                    break
            Vmc=[v for v in V if v!=c]
            Gsubmc=G.subgraph(Vmc)
            u=V[-_sage_const_1 ]
            CC=[v for v in Vmc if Gsubmc.degree(v)>_sage_const_0 ]
            L=[v for v in Vmc if not (v in CC)]
            if len(L)==_sage_const_0 : #complete directly
                t_seq.append(c)
                if len(CC)==_sage_const_2 :
                    return t_seq
                G=(G*[c])
                L.append(u)
                CC.remove(u)
    ...
    def eulerian_tour(self):
        ...
        if not U_edges:
            return False
        return list(map(lambda x:x[_sage_const_0 ],U_edges))
    def eulerian_tours(self,current=None):
        ...
        if current==None:
            ...
            outer_call=True
        if self.num_edges()==_sage_const_0 :
            return [[]]
        ...
        tours=[]
        if len(non_bridges)>_sage_const_0 :
            ...
            for v in bridges:
                ...
                tours+=self.eulerian_tours(v)
                self.add_edge((current,v))
        map(lambda x:x.insert(_sage_const_0 ,current),tours)
        if outer_call:
    ...
    def to_double_occ_word(self):
        ...
        """
        for P in Permutations(range(len(self))*_sage_const_2 ):
            m=double_occ_word(P)
            G=m.to_circle_graph()
            if self==G:
                return m
    ...
    def multi_flow(self,sources,targets):
        ...
        try:
            ...
            try:
                ...
                pass
        return map(lambda x:x[_sage_const_1 :-_sage_const_1 ],paths)
    def is_DH(self):
        ...
        while change:
            ...
            for i in range(n):
                ...
                if change:
                    break
        return G.num_edges()==_sage_const_0 
    ...
    def nu(self,x):
        ...
        try:
            n=self.num_verts()
            vec=vector(GF(_sage_const_2 ),[_sage_const_0 ]*n)
            for e in x:
    ...
    def _nu_internal(self,e):
        ...
        """
        nx=self.neigborhood_vector(e[_sage_const_0 ])
        ny=self.neigborhood_vector(e[_sage_const_1 ])
    ...
    def cycle_space(self):
        ...
        for Cb in self.cycle_basis(output='edge'):
            bas_vec=vector(GF(_sage_const_2 ),[((e in Cb) or ((e[_sage_const_1 ],e[_sage_const_0 ],None) in Cb)) for e in edges])
            basis_vecs.append(bas_vec)
        V=VectorSpace(GF(_sage_const_2 ),len(edges))
    ...
    def cycles(self):
        ...
        for vec in cycle_space:
            C=[edges[i] for i in range(len(edges)) if vec[i]==_sage_const_1 ]
            cycles.append(C)
        return cycles
    def bineighborhood_space(self):
        """Returns the bineighborhood space as defined by Bouchet"""
        V=VectorSpace(GF(_sage_const_2 ),self.num_verts())
    ...
    def k_index(self):
        ...
        if self.is_mu():
            return _sage_const_2 **orthog_dim+_sage_const_2 
        else:
            return _sage_const_2 **orthog_dim
    def number_of_eulerian_vectors(self):
        """Returns the number of Eulerian vectors (slow)"""
        if self.num_verts()==_sage_const_0 :
            return _sage_const_1 
        v=self.random_vertex()
        if self.degree(v)==_sage_const_0 :
            return _sage_const_2 *(self-v).number_of_eulerian_vectors()
        w=self.neighbors(v)[_sage_const_0 ]
        G1=self-v
        G2=(self*[v])-v
        G3=(self*[v,w,v])-v
...
class four_regular(SimpleGraph):
    def __init__(self,*args,**kwargs):
        ...
        if not all(map(lambda v:self.degree(v)==_sage_const_4 ,self.vertices())):
    ...
    def merge_transition(self,v,e1,e2):
        ...
        e1=e1[:_sage_const_2 ]
        e2=e2[:_sage_const_2 ]
        es=list(map(lambda e:e[:_sage_const_2 ],[e for e in self.edges() if (v in e)]))
        if len(es)==_sage_const_3 :
            neighbors=Set(self.neighbors(v))
            new_e=list(neighbors-Set([v]))
            self.delete_vertex(v)
            if len(new_e)==_sage_const_1 :
                self.add_edge((new_e[_sage_const_0 ],new_e[_sage_const_0 ]))
            elif len(new_e)==_sage_const_2 :
                self.add_edge(new_e)
            else:
                raise RuntimeError("new e is > 2")
        elif len(es)==_sage_const_4 :
            try:
                es.remove(e1)
            except ValueError:
                es.remove(e1[::-_sage_const_1 ])
            try:
                es.remove(e2)
            except ValueError:
                es.remove(e2[::-_sage_const_1 ])
            (e3,e4)=es
            self.delete_vertex(v)
            new_e1=list(Set(e1+e2)-Set([v]))
            new_e2=list(Set(e3+e4)-Set([v]))
            for e in [new_e1,new_e2]:
                if len(e)==_sage_const_1 :
                    self.add_edge((e[_sage_const_0 ],e[_sage_const_0 ]))
                elif len(e)==_sage_const_2 :
                    self.add_edge(e)
                else:
                    raise RuntimeError("new e is > 2")
        else:
...
class double_occ_word(object):
    def __init__(self,m):
        ...
        try:
            ...
            self._V=set(self._m)
            self._n=len(self._V)
            if not all([self._m.count(v)==_sage_const_2  for v in self._V]):
                raise ValueError("All elements have to occur exactly twice")
        except TypeError:
            self._n=int(m)
            S=SymmetricGroup(_sage_const_2 *self._n)
            perm=Permutation(S.random_element())
            self._m=perm.action(range(self._n)*_sage_const_2 )
            self._V=set(self._m)
    def __repr__(self):
        return str(self._m)
    def __len__(self):
    ...
    def __eq__(self,other):
        ...
        v=self[_sage_const_0 ]
        (s,e)=other.indices(v)
        for i in [s,e]:
            if self._m==(other[i:]+other[:i]):
                return True
            if self._m==(other[i::-_sage_const_1 ]+other[:i:-_sage_const_1 ]):
                return True
        return False
    ...
    def tau(self,v):
        ...
        (s,e)=self.indices(v)
        return double_occ_word(self[:s+_sage_const_1 ]+self[e-_sage_const_1 :s:-_sage_const_1 ]+self[e:])
    def delete_vertex(self,v):
        (s,e)=self.indices(v)
        self._m.pop(s)
        self._m.pop(e)
        self._V.remove(v)
        self._n-=_sage_const_1 
    def delete_vertices(self,verts):
        for v in verts:
            self.delete_vertex(v)
    def indices(self,v):
        s=self._m.index(v)
        e=s+_sage_const_1 +self[s+_sage_const_1 :].index(v)
        return (s,e)
    ...
    def alternances(self):
        ...
        alt=Set()
        for (u,v) in Subsets(self._V,_sage_const_2 ):
    ...
    def to_4regular_graph(self):
        ...
        if self._n>_sage_const_1 :
            G=SimpleGraph(graphs.CycleGraph(_sage_const_2 *self._n))
            ...
            for v in self._V:
                ...
                G.add_edges([(ind[_sage_const_0 ],ind[_sage_const_0 ]) for _ in range(loops)])
                perm[ind[_sage_const_0 ]]=v
            G.relabel(perm)
            G.set_pos(None)
            return four_regular(G)
        else:
            v=self._m[_sage_const_0 ]
            G=SimpleGraph([(v,v),(v,v)],format='list_of_edges',loops=True,multiedges=True)
            return four_regular(G)

class K_object(object):
    _f=GF(_sage_const_4 ,'w')
    _w=_f.gen()
    def __init__(self,a):
        """An element of GF(4)"""
        try:
            self._a=(GF(_sage_const_2 )(a[_sage_const_0 ]),GF(_sage_const_2 )(a[_sage_const_1 ]))
        except:
            try:
                self._a=(GF(_sage_const_2 )(a._a[_sage_const_0 ]),GF(_sage_const_2 )(a._a[_sage_const_1 ]))
            except:
                if a=='I':
                    self._a=(GF(_sage_const_2 )(_sage_const_0 ),GF(_sage_const_2 )(_sage_const_0 ))
                elif a=='X':
                    self._a=(GF(_sage_const_2 )(_sage_const_1 ),GF(_sage_const_2 )(_sage_const_0 ))
                elif a=='Z':
                    self._a=(GF(_sage_const_2 )(_sage_const_0 ),GF(_sage_const_2 )(_sage_const_1 ))
                elif a=='Y':
                    self._a=(GF(_sage_const_2 )(_sage_const_1 ),GF(_sage_const_2 )(_sage_const_1 ))
                else:
                    raise TypeError("a should be consist of two elements of GF(2), a GF(4) element or be 'I','X','Z' or 'Y'")
    def __repr__(self):
        if self._a[_sage_const_0 ]==_sage_const_0  and self._a[_sage_const_1 ]==_sage_const_0 :
            return 'I'
        elif self._a[_sage_const_0 ]==_sage_const_1  and self._a[_sage_const_1 ]==_sage_const_0 :
            return 'X'
        elif self._a[_sage_const_0 ]==_sage_const_0  and self._a[_sage_const_1 ]==_sage_const_1 :
            return 'Z'
        else:
            return 'Y'
    def __add__(self,other):
    ...
    def __pow__(self,other):
        ...
        if self==_sage_const_0  or other==_sage_const_0  or self==other:
            return GF(_sage_const_2 )(_sage_const_0 )
        else:
            return GF(_sage_const_2 )(_sage_const_1 )
    def __mul__(self,other):
        try:
            a=self.to_gf4()*other
            return K_object(((K_object._w**_sage_const_2 *a).trace(),a.trace()))
        except:
    ...
    def __iter__(self):
        return iter(self._a)
    def to_vector(self):
        return vector(GF(_sage_const_2 ),self._a)
    def to_gf4(self):
        return K_object._f(self._a[_sage_const_0 ]*_sage_const_1 +self._a[_sage_const_1 ]*K_object._w)
...
class K_vec(object):
    def __init__(self,vec):
        ...
        try:
            ...
            try:
                self._vec=tuple(K_object((vec[_sage_const_2 *i],vec[_sage_const_2 *i+_sage_const_1 ])) for i in range(len(vec)/_sage_const_2 ))
            except:
    ...
    def __pow__(self,other):
        ...
        return sum([self[i]**other[i] for i in range(len(self))],GF(_sage_const_2 )(_sage_const_0 ))
    def __repr__(self):
        ...
        if type(P)==type(Set([])):
            ...
            for i in range(len(self)):
                if i in P:
                    ...
                    vec+=(K_object((_sage_const_0 ,_sage_const_0 )),)
            return K_vec(vec)
        else:
            try:
    ...
    def to_vector(self):
        return vector(GF(_sage_const_2 ),sum([tuple(a) for a in self],()))
    def is_complete(self):
        """Are all elements non-zero"""
        return all([a!=_sage_const_0  for a in self])
    def is_supplementary(self,other):
        """Is the pairwise symplectic product (1,....,1)"""
        return all([self[i]**other[i]==_sage_const_1  for i in range(len(self))])
    def vec_hat(self):
        V=VectorSpace(GF(_sage_const_2 ),_sage_const_2 *len(self))
...
class K_mat(object):
    ...
    def __repr__(self):
        ...
        for row in self._mat:
            to_ret+="["+str(row)[_sage_const_1 :-_sage_const_1 ]+"]\n"
        return to_ret[:-_sage_const_1 ]
    def __add__(self,other):
    ...
    def __getitem__(self,i):
        try:
            ...
            try:
                return K_any([vec[i[_sage_const_1 ]] for vec in self._mat[i[_sage_const_0 ]]])
            except:
                return K_any(self._mat[i[_sage_const_0 ]][i[_sage_const_1 ]])
    def __setitem__(self,i,row):
        self._mat[i]=row
    def to_matrix(self):
        return matrix(GF(_sage_const_2 ),[vec.to_vector() for vec in self])
    def transpose(self):
        return K_mat([[self[i,j] for i in range(len(self))] for j in range(len(self[_sage_const_0 ]))])
...
class K(object):
    ...
    def __iter__(self):
        if self._n==None and self._m==None:
            return (K_object(x) for x in VectorSpace(GF(_sage_const_2 ),_sage_const_2 ))
        elif self._m==None:
...
class euler_vec(K_vec):
    ...
    def __mul__(self,m):
        ...
        try:
            for v in m:
                i=S.V().index(int(v))
                current=new[i]
                possible=[x for x in map(K_object,[(_sage_const_1 ,_sage_const_0 ),(_sage_const_0 ,_sage_const_1 ),(_sage_const_1 ,_sage_const_1 )]) if x!=current]
                new[i]=possible[_sage_const_0 ]
                if not self._S.is_eulerian(new):
                    new[i]=possible[_sage_const_1 ]
        except TypeError:
            i=S.V().index(m)
            current=new[i]
            possible=[x for x in map(K_object,[(_sage_const_1 ,_sage_const_0 ),(_sage_const_0 ,_sage_const_1 ),(_sage_const_1 ,_sage_const_1 )]) if x!=current]
    ...
    def __div__(self,other):
        ...
        return "".join(map(str,right_m+left_m[::-_sage_const_1 ]))
    def S(self):
        return self._S
    def V(self):
        return self._S._V
...
class Isotropic(object):
    def __init__(self,G,Ain=None,Bin=None):
        ...
        try:
            ...
            if Ain==None:
                if Bin==None:
                    self._A=K_vec([K_object((_sage_const_0 ,_sage_const_1 ))]*self._n)
                    self._B=K_vec([K_object((_sage_const_1 ,_sage_const_0 ))]*self._n)
                else:
    ...
    def is_isotropy(self):
        ...
        if not self._is_isotropy==None:
            ...
            for vec1 in self:
                for vec2 in self:
                    if not vec1**vec2==_sage_const_0 :
    ...
    def is_eulerian(self,vin):
        ...
        if not v.is_complete():
            return False
        if self.rank_of_vec(v)==_sage_const_0 :
            return True
        return False
    def eulerian_vectors(self):
        ...
        if not self._eulerian_vectors==None:
            ...
            eul=[]
            for v in GF(_sage_const_2 )**(_sage_const_2 *len(self)):
