...
# the tutorial file
`tutorial-1.ipynb` is the file that shows how the aforementioned funcitons could be used to find MERs. It plots the input graph, approximate MER from `SA` and the exact MER from `SA+ILP`. It also prints the runtime of the `SA+ILP` and `ILP` algorithms.
